...
- Test:
  ...
  - from **CLI**, if it is already built in **CLI** or **VSCode** (MTP v2):
    - any OS or shell:
      ...
      ```bash
      dotnet test tests/Domain.Tests/bin/Debug/net10.0/Domain.Tests.dll
      ```

    - on Windows **CLI** (already built in **CLI** or **VSCode** - on MTP v2):

      ```batch
      tests/Domain.Tests/bin/Debug/net10.0/Domain.Tests.exe
      ```

    - on Linux or MacOS **CLI** (already built in **CLI** or **VSCode** - on MTP v2):

      ```bash
      tests/Domain.Tests/bin/Debug/net10.0/Domain.Tests
      ```
...
- Benchmarks (if included):
  ...
  ```bash
  dotnet run --project benchmarks/Domain.Benchmarks/Domain.Benchmarks.csproj --configuration Release
  ```
...
- Package ID: `vm2.Domain`
- Version: 0.0.0
- License: MIT
- Repository: <https://github.com/vmelamed/vm2.Domain>
...
- src/Domain: the library source code
- tests/Domain.Tests: xUnit + MTP tests, includes testconfig.json
- benchmarks/Domain.Benchmarks: BenchmarkDotNet suite (optional)
- examples/Domain.Example: minimal console sample (optional)
